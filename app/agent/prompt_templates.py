...
PROMPT_TEMPLATES = {
    AgentState.MEDICAL_KNOWLEDGE: {
        ...
        ⚠️ 严格规则：
        1. 必须基于提供的医学知识上下文回答问题
        2. 回答风格要客观、科普、专业
        3. 对于症状描述，必须强调"这只是初步判断，最终需要医生面诊确诊"
        4. 禁止做出具体诊断，禁止承诺疗效
        5. 如果上下文没有相关信息，直接说"根据我的知识库，无法回答您的问题"
        6. 不要提及"根据文档"、"根据上下文"等字样
        """,
        "user": "医学知识：\n{context}\n\n用户问题：{query}",
        "name": "MEDICAL_PROMPT"
    },
    ...
    AgentState.USER_DECISION: {
        ...
        ⚠️ 严格规则：
        1. 必须基于提供的决策支持信息回答问题
        2. 回答风格要分析性、引导性，帮助用户消除顾虑
        3. 对于价格问题，提供区间参考并解释价格构成
        4. 对于对比问题，提供优缺点对比并给出建议
        5. 对于风险问题，给出科学概率说明和术后维护建议
        6. 引导用户到院进行专业咨询
        7. 如果上下文没有相关信息，直接说"根据我的知识库，无法回答您的问题"
        """,
        "user": "决策支持信息：\n{context}\n\n用户问题：{query}",
    ...
    AgentState.OBJECTION_HANDLING: {
        ...
        1. 必须基于提供的信息回答问题
        2. 回答风格要同理心、安抚性、专业性
        3. 对于恐惧问题，介绍无痛技术和舒适化诊疗流程
        4. 对于价格异议，强调性价比、耗材溯源和售后服务价值
        5. 对于信任问题，展示医生资质、成功案例和品牌荣誉
        6. 始终保持积极正面的态度
        7. 如果上下文没有相关信息，直接说"根据我的知识库，无法回答您的问题"
        """,
        "user": "异议处理信息：\n{context}\n\n用户问题：{query}",
